...
class WalletMgr(object):
    ...
    def __init__(self, walletd, nodbesPort=8888, nodbesHost="localhost", port=9899, host="localhost"):
    ...
    def create(self, name, accounts=None, exitOnError=True):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s --to-console" % (Utils.BesClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
