...
class Node(object):
    ...
    def getBlock(self, blockNum, silentErrors=False):
        ...
        if not self.enableMongo:
            cmd="%s %s get block %d" % (Utils.BesClientPath, self.endpointArgs, blockNum)
    ...
    def getBesAccount(self, name):
        ...
        if not self.enableMongo:
            ...
            return self.getBesAccountFromDb(name)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False):
        ...
        cmd="%s %s %s -j %s %s \"%s %s\" \"%s %s\" %s" % (
            Utils.BesClientPath, self.endpointArgs, specificCmd, fromAccount.name, toAccount.name, netQuantity, CORE_SYMBOL, cpuQuantity, CORE_SYMBOL, transferStr)
    ...
    def regproducer(self, producer, url, location, waitForTransBlock=False):
        ...
        cmd="%s %s %s -j %s %s %s %s" % (
            Utils.BesClientPath, self.endpointArgs, specificCmd, producer.name, producer.activePublicKey, url, location)
        return self.processCmd(cmd, specificCmd, waitForTransBlock)

    def vote(self, account, producers, waitForTransBlock=False):
        specificCmd = "system voteproducer prods"
        cmd="%s %s %s -j %s %s" % (
            Utils.BesClientPath, self.endpointArgs, specificCmd, account.name, " ".join(producers))
        return self.processCmd(cmd, specificCmd, waitForTransBlock)
