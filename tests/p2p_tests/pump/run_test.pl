...
my $bes_home = defined $ENV{BES_HOME} ? $ENV{BES_HOME} : getcwd;
my $besd = $bes_home . "/programs/besd/besd";
my $besc = $bes_home . "/programs/besc/besc";

my $nodes = defined $ENV{BES_TEST_RING} ? $ENV{BES_TEST_RING} : "1";
my $pnodes = defined $ENV{BES_TEST_PRODUCERS} ? $ENV{BES_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$bes_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of besd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"BES6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = besio::producer_plugin\n";
        print $cfg "plugin = besio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$besc transfer bes inita 10 >> besc.out 2>> besc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
