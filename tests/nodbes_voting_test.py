...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.besioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    if testWallet is None:
        Utils.cmdError("bes wallet create")
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.besioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000)
        if trans is None:
        ...
        if node.transferFunds(cluster.besioAccount, account, transferAmount, "test transfer") is None:
            errorExit("Failed to transfer funds %d from account %s to %s" % (
                transferAmount, cluster.besioAccount.name, account.name))
