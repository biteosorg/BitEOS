...
killAll()
{
  programs/besio-launcher/besio-launcher -k 15
}

cleanup()
{
    rm -rf etc/besio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clbes/clbes get info)"
  verifyErrorCode "clbes get info"
...
# stand up nodbes cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodbes \"--plugin besio::wallet_api_plugin\" $launcherOpts
programs/besio-launcher/besio-launcher --nodbes "--plugin besio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clbes/clbes --port $port2 get block 1)"
    verifyErrorCode "clbes get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clbes/clbes create key)"
verifyErrorCode "clbes create key"
...
PASSWORD_INITA="$(programs/clbes/clbes wallet create --name inita)"
verifyErrorCode "clbes wallet create"
...
ACCOUNT_INFO="$(programs/clbes/clbes create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clbes create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clbes/clbes get account testera)"
verifyErrorCode "clbes get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clbes/clbes transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clbes transfer"
   ...
   TRANS_INFO="$(programs/clbes/clbes --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clbes get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
