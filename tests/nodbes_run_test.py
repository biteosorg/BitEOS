...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.besioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    transId=node.createInitializeAccount(testeraAccount, cluster.besioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.besioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.besioAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.besioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.besioAccount, waitForTransBlock=True, exitOnError=True)
    ...
    wasmFile="besio.token.wasm"
    abiFile="besio.token.abi"
    ...
    account=node.getBesAccount(defproduceraAccount.name, exitOnError=True)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killBesInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
