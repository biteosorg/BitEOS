...
###############################################################
# nodbes_run_test
# --dump-error-details <Upon error print etc/besio/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
parser.add_argument("-h", "--host", type=str, help="%s host name" % (testUtils.Utils.BesServerName),
                    default=LOCAL_HOST)
parser.add_argument("-p", "--port", type=int, help="%s host port" % testUtils.Utils.BesServerName,
...
parser.add_argument("--dump-error-details",
                    help="Upon error print etc/besio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                    action='store_true')
...
parser.add_argument("--kill-all", help="Kill all nodbes and klbes instances", action='store_true')
...
try:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up bes walletd.")
    ...
    # create accounts via besio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.besioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.besioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.besioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.besioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.besioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.besioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    actualAmount=node.getAccountBesBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountBesBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/besio.token"
    wastFile="contracts/besio.token/besio.token.wast"
    abiFile="contracts/besio.token/besio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getBesAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get BES account failed", raw=True)
    ...
    account=node.getBesAccount(defproduceraAccount.name)
    if account is None:
