...
# --kill-count <nodbes instances to kill>
...
parser.add_argument("--kill-count", type=int, help="nodbes instances to kill", default=-1)
...
killBesInstances= not args.dont_kill
...
try:
    ...
    Print("Stand up BES wallet kbesd")
    walletMgr.killall(allInstances=killAll)
    walletMgr.cleanup()
    if walletMgr.launch() is False:
        errorExit("Failed to stand up kbesd.")
    ...
    besioAccount=cluster.besioAccount
    ...
    if not cluster.createAccounts(besioAccount):
    ...
    if cluster.killSomeBesInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Bes instances")
    Print("nodbes instances killed.")
    ...
    if cluster.relaunchBesInstances() is False:
        errorExit("Failed to relaunch Bes instances")
    Print("nodbes instances relaunched.")
