...
class Utils:
    ...
    BesClientPath="programs/clbes/clbes"

    BesWalletName="kbesd"
    BesWalletPath="programs/kbesd/"+ BesWalletName

    BesServerName="nodbes"
    BesServerPath="programs/nodbes/"+ BesServerName

    BesLauncherPath="programs/besio-launcher/besio-launcher"
    MongoPath="mongo"
    ...
    # mongoSyncTime: nodbes mongodb plugin seems to sync with a 10-15 seconds delay. This will inject
...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="BEStest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getBesAccount(account.name)
            try:
                assert(accountInfo)
    ...
    def getBlock(self, blockNum, retry=True, silentErrors=False):
        ...
        if not self.enableMongo:
            cmd="%s %s get block %s" % (Utils.BesClientPath, self.endpointArgs, blockNum)
    ...
    def getTransaction(self, transId, retry=True, silentErrors=False):
        if not self.enableMongo:
            cmd="%s %s get transaction %s" % (Utils.BesClientPath, self.endpointArgs, transId)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                trans=Utils.runCmdReturnJson(cmd)
                return trans
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False):
        cmd='%s %s system newaccount -j %s %s %s %s --stake-net "100 %s" --stake-cpu "100 %s" --buy-ram "100 %s"' % (
            Utils.BesClientPath, self.endpointArgs, creatorAccount.name, account.name,
    ...
    def getBesAccount(self, name):
        assert(isinstance(name, str))
        cmd="%s %s get account -j %s" % (Utils.BesClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getBesAccountFromDb(self, name):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table):
        cmd="%s %s get table %s %s %s" % (Utils.BesClientPath, self.endpointArgs, contract, scope, table)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL):
        ...
        cmd="%s %s get currency balance %s %s %s" % (Utils.BesClientPath, self.endpointArgs, contract, account, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getCurrencyStats(self, contract, symbol=CORE_SYMBOL):
        ...
        assert(symbol)
        assert(isinstance(symbol, str))
        cmd="%s %s get currency stats %s %s" % (Utils.BesClientPath, self.endpointArgs, contract, symbol)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def verifyAccount(self, account):
        if not self.enableMongo:
            ret=self.getBesAccount(account.name)
            if ret is not None:
            ...
            for _ in range(2):
                ret=self.getBesAccountFromDb(account.name)
                if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.BesClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getBesBalances([source] + accounts)
    ...
    def getBesBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountBesBalance(account.name)
    ...
    def getAccountsByKey(self, key):
        cmd="%s %s get accounts %s" % (Utils.BesClientPath, self.endpointArgs, key)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    # Get actions mapped to an account (clbes get actions)
    def getActions(self, account, pos=-1, offset=-1):
        assert(isinstance(account, Account))
        assert(isinstance(pos, int))
        assert(isinstance(offset, int))

        cmd="%s %s get actions -j %s %d %d" % (Utils.BesClientPath, self.endpointArgs, account.name, pos, offset)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getServants(self, name):
        cmd="%s %s get servants %s" % (Utils.BesClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=Utils.runCmdReturnJson(cmd)
            return trans
    ...
    def getAccountBesBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clbes get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        if not self.enableMongo:
            amount=self.getTableAccountBalance("besio.token", scope)
            if Utils.Debug: Utils.Print("getNodeAccountBesBalance %s %s" % (scope, amount))
            ...
            account=self.getBesAccountFromDb(scope)
            if account is not None:
                balance=account["bes_balance"]
                return balance

        return None

    def getAccountBesBalance(self, scope):
        """Returns SYS currency0000 account balance from clbes get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountBesBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.BesClientPath, self.endpointArgs, account)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            retStr=Utils.checkOutput(cmd.split())
    ...
    def publishContract(self, account, contractDir, wastFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.BesClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.BesClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False):
        cmd="%s %s set action permission -j %s %s %s %s" % (
            Utils.BesClientPath, self.endpointArgs, account, code, pType, requirement)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
            trans=Utils.runCmdReturnJson(cmd)
    ...
    def getInfo(self, silentErrors=False):
        cmd="%s %s get info" % (Utils.BesClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clbes get info."""
        if not self.enableMongo:
            info=self.getInfo()
            if info is not None:
...
class WalletMgr(object):
    __walletLogFile="test_kbesd_output.log"
    __walletDataDir="test_wallet_0"

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kbesd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodbesPort=8888, nodbesHost="localhost", port=8899, host="localhost"):
        self.walletd=walletd
        self.nodbesPort=nodbesPort
        self.nodbesHost=nodbesHost
        self.port=port
        self.host=host
        self.wallets={}
        self.__walletPid=None
        self.endpointArgs="--url http://%s:%d" % (self.nodbesHost, self.nodbesPort)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kbesd")
            return False

        cmd="%s --data-dir %s --config-dir %s --http-server-address=%s:%d" % (
            Utils.BesWalletPath, WalletMgr.__walletDataDir, WalletMgr.__walletDataDir, self.host, self.port)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        with open(WalletMgr.__walletLogFile, 'w') as sout, open(WalletMgr.__walletLogFile, 'w') as serr:
            popen=subprocess.Popen(cmd.split(), stdout=sout, stderr=serr)
            self.__walletPid=popen.pid

        # Give kbesd time to warm up
        time.sleep(1)
        return True
    ...
    def create(self, name):
        ...
        if wallet is not None:
            ...
            return wallet
        p = re.compile(r'\n\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet create --name %s" % (Utils.BesClientPath, self.endpointArgs, name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def importKey(self, account, wallet):
        ...
        cmd="%s %s wallet import --name %s %s" % (
            Utils.BesClientPath, self.endpointArgs, wallet.name, account.ownerPrivateKey)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
        ...
        if account.activePrivateKey is None:
            ...
            cmd="%s %s wallet import --name %s %s" % (
                Utils.BesClientPath, self.endpointArgs, wallet.name, account.activePrivateKey)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            try:
                Utils.checkOutput(cmd.split())
            except subprocess.CalledProcessError as ex:
    ...
    def lockWallet(self, wallet):
        cmd="%s %s wallet lock --name %s" % (Utils.BesClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def unlockWallet(self, wallet):
        cmd="%s %s wallet unlock --name %s" % (Utils.BesClientPath, self.endpointArgs, wallet.name)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def lockAllWallets(self):
        cmd="%s %s wallet lock_all" % (Utils.BesClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
    ...
    def getOpenWallets(self):
        ...
        cmd="%s %s wallet list" % (Utils.BesClientPath, self.endpointArgs)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
    ...
    def getKeys(self, wallet):
        ...
        p = re.compile(r'\n\s+\"(\w+)\"\n', re.MULTILINE)
        cmd="%s %s wallet private_keys --name %s --password %s " % (Utils.BesClientPath, self.endpointArgs, wallet.name, wallet.password)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        retStr=Utils.checkOutput(cmd.split())
        #Utils.Print("retStr: %s" % (retStr))
        m=p.findall(retStr)
    ...
    # @staticmethod
    # def killall():
    #     cmd="pkill -9 %s" % (Utils.BesWalletName)
    #     if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    #     subprocess.call(cmd.split())

    def killall(self, allInstances=False):
        """Kill kbes instances. allInstances will kill all kbes instances running on the system."""
        if self.__walletPid:
...
class Cluster(object):
    ...
    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kbesd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="BEStest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kbesd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: bes server host
        port: bes server port
        walletHost: bes wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures bes mongo plugin
        ...
        self.besioAccount=Account("besio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontKill=False, dontBootstrap=False):
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.BesServerName))
            return True
        ...
        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin bnet" % (
            Utils.BesLauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3])
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodbesArgs="--max-transaction-time 5000 --filter-on *"
        if not self.walletd:
            nodbesArgs += " --plugin besio::wallet_api_plugin"
        if self.enableMongo:
            nodbesArgs += " --plugin besio::mongo_db_plugin --delete-all-blocks --mongodb-uri %s" % self.mongoUri

        if nodbesArgs:
            cmdArr.append("--nodbes")
            cmdArr.append(nodbesArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.BesServerName, totalNodes, len(nodes)))
            return False
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key" % (Utils.BesClientPath)
                if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on bes server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextBesIdx: %d, n: %d" % (nextBesIdx, n))
                nextBesIdx=(nextBesIdx + 1)%count
                if not self.nodes[nextBesIdx].killed:
                    #Utils.Print("nextBesIdx: %d" % (nextBesIdx))
            ...
            #Utils.Print("nextBesIdx: %d, count: %d" % (nextBesIdx, count))
            node=self.nodes[nextBesIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.BesServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on bes node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getBesBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.besioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        node="node_bios"
        configFile="etc/besio/%s/config.ini" % (node)
        if Utils.Debug: Utils.Print("Parsing config file %s" % configFile)
        producerKeys=Cluster.parseProducerKeys(configFile, node)
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse besio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            besioName="besio"
            besioKeys=producerKeys[besioName]
            besioAccount=Account(besioName)
            besioAccount.ownerPrivateKey=besioKeys["private"]
            besioAccount.ownerPublicKey=besioKeys["public"]
            besioAccount.activePrivateKey=besioKeys["private"]
            besioAccount.activePublicKey=besioKeys["public"]

            if not walletMgr.importKey(besioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (besioName))
                return False
            ...
            trans=biosNode.publishContract(besioAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, besioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        opts="--permission besio@active"
                        myTrans=biosNode.pushMessage("besio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("besio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but besio).
                lam = lambda: biosNode.getInfo()["head_block_producer"] != "besio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return False

            besioTokenAccount=copy.deepcopy(besioAccount)
            besioTokenAccount.name="besio.token"
            trans=biosNode.createAccount(besioTokenAccount, besioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (besioTokenAccount.name))
                return False

            besioRamAccount=copy.deepcopy(besioAccount)
            besioRamAccount.name="besio.ram"
            trans=biosNode.createAccount(besioRamAccount, besioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (besioRamAccount.name))
                return False

            besioRamfeeAccount=copy.deepcopy(besioAccount)
            besioRamfeeAccount.name="besio.ramfee"
            trans=biosNode.createAccount(besioRamfeeAccount, besioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (besioRamfeeAccount.name))
                return False

            besioStakeAccount=copy.deepcopy(besioAccount)
            besioStakeAccount.name="besio.stake"
            trans=biosNode.createAccount(besioStakeAccount, besioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (besioStakeAccount.name))
                return False
            ...
            contract="besio.token"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(besioTokenAccount.name, contractDir, wastFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return False

            # Create currency0000, followed by issue currency0000
            contract=besioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (besioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to besio contract.")
                return False
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (besioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to besio contract.")
                return False
            ...
            Utils.Print("Verify besio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountBesBalanceStr(besioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return False

            contract="besio.system"
            contractDir="contracts/%s" % (contract)
            wastFile="contracts/%s/%s.wast" % (contract, contract)
            abiFile="contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (besioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (besioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (besioTokenAccount.name, name))
                    return False
    ...
    # Populates list of BesInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.BesServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.BesServerName, pattern))
                break
    ...
    # Kills a percentange of Bes instances starting from the tail and update besInstanceInfos state
    def killSomeBesInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.BesServerName, killSignal))
    ...
    def dumpErrorDetails(self):
        fileName="etc/besio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/besio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodbes instances. allInstances will kill all nodbes instances running on the system."""
        cmd="%s -k 9" % (Utils.BesLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down bes cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the bes server
            cmd="pkill -9 %s" % (Utils.BesServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down bes cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/besio/node_*"):
            shutil.rmtree(f)
