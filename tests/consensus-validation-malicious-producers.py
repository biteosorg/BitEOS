...
plugin = besio::account_history_plugin
plugin = besio::account_history_api_plugin"""
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/besio/node_%02d" % (count))
...
killBesInstances= not args.dont-kill
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up bes cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up bes walletd.")
            return False
