...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.besioAccount])
    ...
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.besioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.besioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.besioAccount.name, account.name))
        nodes[0].transferFunds(cluster.besioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.besioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.besioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.besioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.besioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    Print("Failed to push create action to besio contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
