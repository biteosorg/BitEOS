...
namespace besio {
   ...
   void set_blockchain_parameters(const besio::blockchain_parameters& params) {
      char buf[sizeof(besio::blockchain_parameters)];
      besio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(besio::blockchain_parameters& params) {
      char buf[sizeof(besio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      besio_assert( size <= sizeof(buf), "buffer is too small" );
      besio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   */
   ...
   private:
      ...
      public:
         ...
         {
            besio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return besio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = besio::memory_heap.malloc(count*size);
...
{
   return besio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return besio::memory_heap.free(ptr);
}
