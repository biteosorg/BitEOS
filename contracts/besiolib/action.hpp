...
namespace besio {
    ...
    *    int  c; //4
    *
    *    BESLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      BESLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      BESLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         besio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         besio_assert( name == T::get_name(), "Invalid name" );
         besio_assert( account == T::get_account(), "Invalid account" );
...
::besio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::besio::string_to_name(#NAME) )
...
#define ACTION( CODE, NAME ) struct NAME : ::besio::action_meta<CODE, ::besio::string_to_name(#NAME) >
