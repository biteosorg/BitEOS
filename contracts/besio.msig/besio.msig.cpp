#include <besio.msig/besio.msig.hpp>
#include <besiolib/action.hpp>
#include <besiolib/permission.hpp>
...
void multisig::propose() {
   ...
   besio_assert( trx_header.expiration >= besio::time_point_sec(now()), "transaction expired" );
   //besio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   besio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   besio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   besio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   besio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      besio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < besio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
BESIO_ABI( besio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
