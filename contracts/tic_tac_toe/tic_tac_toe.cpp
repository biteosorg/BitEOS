...
struct impl {
   ...
   void on(const create& c) {
      require_auth(c.host);
      besio_assert(c.challenger != c.host, "challenger shouldn't be the same as host");

      // Check if game already exists
      games existing_host_games(code_account, c.host);
      auto itr = existing_host_games.find( c.challenger );
      besio_assert(itr == existing_host_games.end(), "game already exists");
   ...
   void on(const restart& r) {
      ...
      besio_assert(itr != existing_host_games.end(), "game doesn't exists");

      // Check if this game belongs to the action sender
      besio_assert(r.by == itr->host || r.by == itr->challenger, "this is not your game!");
   ...
   void on(const move& m) {
      ...
      besio_assert(itr->winner == N(none), "the game has ended!");
      // Check if this game belongs to the action sender
      besio_assert(m.by == itr->host || m.by == itr->challenger, "this is not your game!");
      // Check if this is the  action sender's turn
      besio_assert(m.by == itr->turn, "it's not your turn yet!");


      // Check if user makes a valid movement
      besio_assert(is_valid_movement(m.mvt, *itr), "not a valid movement!");
   ...
   void apply( uint64_t /*receiver*/, uint64_t code, uint64_t action ) {
      ...
      if (code == code_account) {
         if (action == N(create)) {
            impl::on(besio::unpack_action_data<tic_tac_toe::create>());
         } else if (action == N(restart)) {
            impl::on(besio::unpack_action_data<tic_tac_toe::restart>());
         } else if (action == N(close)) {
            impl::on(besio::unpack_action_data<tic_tac_toe::close>());
         } else if (action == N(move)) {
            impl::on(besio::unpack_action_data<tic_tac_toe::move>());
         }
      }
   }
