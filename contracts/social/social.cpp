...
/**
 *  The purpose of this contract is to implement something like Steem on BES, this
...
void apply_social_post() {
   ...
   besio_assert( current_context() == post.author, "cannot call from any other context" );
...
void apply_social_vote() {
   ...
   if( context == vote.author ) {
      ...
      besio_assert( Db::get( vote.postid, post ) > 0, "unable to find post" );
      besio_assert( now() - post.created < days(7), "cannot vote after 7 days" );
      ...
      besio_assert( abs_vote <= vote_account.vote_power, "insufficient vote power" );
      post.votes += vote.vote_power;
      vote_account.vote_power -= abs_vote;
      vote_account.last_vote  = now();
      Db::store( "account", vote_account );
   } else {
      besio_assert( false, "invalid context for execution of this vote" );
   }
}
