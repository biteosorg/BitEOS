...
class dice : public besio::contract {
   public:
      ...
      void offerbet(const asset& bet, const account_name player, const checksum256& commitment) {
         ...
         besio_assert( bet.symbol == CORE_SYMBOL, "only core token allowed" );
         besio_assert( bet.is_valid(), "invalid bet" );
         besio_assert( bet.amount > 0, "must bet positive quantity" );

         besio_assert( !has_offer( commitment ), "offer with this commitment already exist" );
         require_auth( player );

         auto cur_player_itr = accounts.find( player );
         besio_assert(cur_player_itr != accounts.end(), "unknown account");
         ...
         if( matched_offer_itr == idx.end()
            ...
            accounts.modify( cur_player_itr, 0, [&](auto& acnt) {
               besio_assert( acnt.bes_balance >= bet, "insufficient balance" );
               acnt.bes_balance -= bet;
               acnt.open_offers++;
            });
            ...
            auto game_itr = games.emplace(_self, [&](auto& new_game){
               ...
               new_game.deadline = besio::time_point_sec(0);
      ...
      void canceloffer( const checksum256& commitment ) {
         ...
         besio_assert( offer_itr != idx.end(), "offer does not exists" );
         besio_assert( offer_itr->gameid == 0, "unable to cancel offer" );
         require_auth( offer_itr->owner );

         auto acnt_itr = accounts.find(offer_itr->owner);
         accounts.modify(acnt_itr, 0, [&](auto& acnt){
            acnt.open_offers--;
            acnt.bes_balance += offer_itr->bet;
         });
      ...
      void reveal( const checksum256& commitment, const checksum256& source ) {
         ...
         besio_assert(curr_revealer_offer != idx.end(), "offer not found");
         besio_assert(curr_revealer_offer->gameid > 0, "unable to reveal");
         ...
         besio_assert( is_zero(curr_reveal.reveal) == true, "player already revealed");
         ...
         if( !is_zero(prev_reveal.reveal) ) {
            ...
            games.modify(game_itr, 0, [&](auto& game){
               ...
               game.deadline = besio::time_point_sec(now() + FIVE_MINUTES);
            });
         }
      }
      ...
      void claimexpired( const uint64_t gameid ) {
         ...
         besio_assert(game_itr != games.end(), "game not found");
         besio_assert(game_itr->deadline != besio::time_point_sec(0) && besio::time_point_sec(now()) > game_itr->deadline, "game not expired");
         ...
         if( !is_zero(game_itr->player1.reveal) ) {
            besio_assert( is_zero(game_itr->player2.reveal), "game error");
            pay_and_clean(*game_itr, *player1_offer, *player2_offer);
         } else {
            besio_assert( is_zero(game_itr->player1.reveal), "game error");
      ...
      void deposit( const account_name from, const asset& quantity ) {
         ...
         besio_assert( quantity.amount > 0, "must deposit positive quantity" );
         ...
         action(
            ...
            N(besio.token), N(transfer),
            std::make_tuple(from, _self, quantity, std::string(""))
         ).send();

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            acnt.bes_balance += quantity;
         });
      }
      ...
      void withdraw( const account_name to, const asset& quantity ) {
         ...
         besio_assert( quantity.is_valid(), "invalid quantity" );
         besio_assert( quantity.amount > 0, "must withdraw positive quantity" );

         auto itr = accounts.find( to );
         besio_assert(itr != accounts.end(), "unknown account");

         accounts.modify( itr, 0, [&]( auto& acnt ) {
            besio_assert( acnt.bes_balance >= quantity, "insufficient balance" );
            acnt.bes_balance -= quantity;
         });
   ...
   private:
      ...
      struct offer {
         ...
         BESLIB_SERIALIZE( offer, (id)(owner)(bet)(commitment)(gameid) )
      };

      typedef besio::multi_index< N(offer), offer,
      ...
      struct player {
         ...
         BESLIB_SERIALIZE( player, (commitment)(reveal) )
      };
      ...
      struct game {
         uint64_t id;
         asset    bet;
         besio::time_point_sec deadline;
         player   player1;
         player   player2;

         uint64_t primary_key()const { return id; }

         BESLIB_SERIALIZE( game, (id)(bet)(deadline)(player1)(player2) )
      };

      typedef besio::multi_index< N(game), game> game_index;
      ...
      struct global_dice {
         ...
         BESLIB_SERIALIZE( global_dice, (id)(nextgameid) )
      };

      typedef besio::multi_index< N(global), global_dice> global_dice_index;
      ...
      struct account {
         ...
         bool is_empty()const { return !( bes_balance.amount | open_offers | open_games ); }

         uint64_t primary_key()const { return owner; }

         BESLIB_SERIALIZE( account, (owner)(bes_balance)(open_offers)(open_games) )
      };

      typedef besio::multi_index< N(account), account> account_index;
...
BESIO_ABI( dice, (offerbet)(canceloffer)(reveal)(claimexpired)(deposit)(withdraw) )
