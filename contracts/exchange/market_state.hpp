...
namespace besio {
   ...
   struct margin_position {
      ...
      BESLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef besio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), besio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      BESLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef besio::multi_index<N(loans), loan_position> loans;
