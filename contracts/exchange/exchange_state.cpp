...
namespace besio {
   ...
   extended_asset exchange_state::convert_from_exchange( connector& c, extended_asset in ) {
      besio_assert( in.contract == supply.contract, "unexpected asset contract input" );
      besio_assert( in.symbol== supply.symbol, "unexpected asset symbol input" );
   ...
   extended_asset exchange_state::convert( extended_asset from, extended_symbol to ) {
      ...
      if( sell_symbol != ex_symbol ) {
         if( sell_symbol == base_symbol ) {
            ...
            besio_assert( false, "invalid sell" );
         }
      } else {
         if( to == base_symbol ) {
            ...
            besio_assert( false, "invalid conversion" );
         }
      }
