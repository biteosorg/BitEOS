...
namespace besio {
   ...
   {
      besio_assert( market_state_itr != market_table.end(), "unknown market" );
   ...
   void market_state::margin_call( exchange_state::connector& c, margins& marginstable ) {
      ...
      besio_assert( receipt.amount >= pos->borrowed.amount, "programmer error: insufficient collateral to cover" );/// VERY BAD, SHOULD NOT HAPPEN
   ...
   void market_state::lend( account_name lender, const extended_asset& quantity ) {
      ...
      else besio_assert( false, "unable to lend to this market" );
   }

   void market_state::unlend( account_name lender, double ishares, const extended_symbol& sym ) {
      besio_assert( ishares > 0, "cannot unlend negative balance" );
   ...
   void market_state::adjust_lend_shares( account_name lender, loans& l, double delta ) {
      ...
      if( existing == l.end() ) {
         l.emplace( lender, [&]( auto& obj ) {
            ...
            besio_assert( delta >= 0, "underflow" );
         });
      } else {
         l.modify( existing, 0, [&]( auto& obj ) {
            obj.interest_shares += delta;
            besio_assert( obj.interest_shares >= 0, "underflow" );
         });
      }
   }
   ...
   void market_state::cover_margin( account_name borrower, const extended_asset& cover_amount ) {
      if( cover_amount.get_extended_symbol() == exstate.base.balance.get_extended_symbol() ) {
         ...
         besio_assert( false, "invalid debt asset" );
      }
   }
                                    ...
                                    const extended_asset& cover_amount )
   {
      ...
      besio_assert( existing != m.end(), "no known margin position" );
      besio_assert( existing->borrowed.amount >= cover_amount.amount, "attempt to cover more than user has" );
      ...
      besio_assert( debpaid.amount >= cover_amount.amount, "unable to cover debt" );
                                     ...
                                     const extended_asset& delta_debt, const extended_asset& delta_col )
   {
      auto existing = m.find( borrower );
      if( existing == m.end() ) {
         besio_assert( delta_debt.amount > 0, "cannot borrow neg" );
         besio_assert( delta_col.amount > 0, "cannot have neg collat" );
         ...
         if( existing->borrowed.amount == -delta_debt.amount ) {
            besio_assert( existing->collateral.amount == -delta_col.amount, "user failed to claim all collateral" );
      ...
      besio_assert( c.peer_margin.total_lent.amount <= c.peer_margin.total_lendable.amount, "insufficient funds availalbe to borrow" );

      if( existing != m.end() ) {
         if( existing->call_price < c.peer_margin.least_collateralized )
            c.peer_margin.least_collateralized = existing->call_price;

         besio_assert( !exstate.requires_margin_call( c ), "this update would trigger a margin call" );
      } else {
         c.peer_margin.least_collateralized = std::numeric_limits<double>::max();
      }
