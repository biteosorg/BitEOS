...
class simpletoken : public besio::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         besio_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      besio::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              besio_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

BESIO_ABI( simpletoken, (transfer)(issue) )
