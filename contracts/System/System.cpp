...
void system_contract::update_elected_bps() {
  ...
  std::vector<besio::producer_key> vote_schedule;
...
void system_contract::transfer( const account_name from, const account_name to, const asset quantity, const string memo ) {
  ...
  besio_assert( quantity.symbol == SYMBOL, "only support BES which has 4 precision" );
  //from_act.available is already handling fee
  besio_assert( 0 <= quantity.amount && quantity.amount <= from_act.available.amount, "need 0.0000 BES < quantity < available balance" );
  besio_assert( memo.size() <= 256, "memo has more than 256 bytes" );
...
void system_contract::updatebp( const account_name bpname, const public_key block_signing_key,
  ...
  require_auth( bpname );
  besio_assert( url.size() < 64, "url too long" );
  besio_assert( 1 <= commission_rate && commission_rate <= 10000, "need 1 <= commission rate <= 10000" );
...
void system_contract::vote( const account_name voter, const account_name bpname, const asset stake ) {
  ...
  besio_assert( stake.symbol == SYMBOL, "only support BES which has 4 precision" );
  besio_assert( 0 <= stake.amount && stake.amount % 10000 == 0, "need stake quantity >= 0.0000 BES and quantity is integer" );
  ...
  if( vts == votes_tbl.end() ){
    ...
    besio_assert( stake.amount <= act.available.amount, "need stake quantity < your available balance" );
    ...
    //act.available is already handling fee
    besio_assert( change <= act.available.amount, "need stake change quantity < your available balance" );
...
void system_contract::unfreeze( const account_name voter, const account_name bpname ){
  ...
  besio_assert( vts.unstake_height + FROZEN_DELAY < current_block_num(), "unfreeze is not available yet" );
  besio_assert( 0 < vts.unstaking.amount , "need unstaking quantity > 0.0000 BES" );
...
void system_contract::claim( const account_name voter, const account_name bpname ){
  ...
  besio_assert( 0 < newest_total_voteage, "claim is not available yet" );
  ...
  besio_assert( 0 <= reward.amount && reward.amount <= bp.rewards_pool.amount, "need 0 <= claim reward quantity <= rewards_pool" );
...
void system_contract::onblock( const block_timestamp, const account_name bpname, const uint16_t, const block_id_type,
  ...
  besio_assert( 0 <= commission.amount && commission.amount <= BLOCK_REWARDS_BP, "need 0 <= commission reward quantity <= bp block rewards" );
...
void system_contract::onfee( const account_name actor, const asset fee, const account_name bpname ) {
  print( "-----onfee-----", besio::name{.value=actor}, "-----", fee, "\n" );

  accounts_table acnts_tbl( _self, _self );
  const auto & act = acnts_tbl.get( actor, "account is not found in accounts table" );
  besio_assert( fee.amount <= act.available.amount, "overdrawn available balance" );
