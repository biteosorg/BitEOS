...
namespace besiosystem {
   ...
   struct user_resources {
      ...
      BESLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(besio::token, transfer)( N(besio.token), {payer,N(active)},
                                                       { payer, N(besio.ramfee), fee, std::string("ram fee") } );
      }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      besio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      // update voting power
      {
         ...
         besio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
                                       ...
                                       asset unstake_net_quantity, asset unstake_cpu_quantity )
   {
      ...
      besio_assert( _gstate.total_activated_stake >= min_activated_stake, "not enough has been staked for users to unstake" );
