...
public:
   ...
   static abi_def bes_abi; // cached for common use
                ...
                const chain::action& msg)
  {
     ...
     try {
        ...
        if (msg.account == chain::config::system_account_name) {
           abi = chain::besio_contract_abi(abi);
        }
        abis.set_abi(abi);
        ...
        try {
           ...
           elog("Unable to convert BES JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("  BES JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
// For now providing some simple account processing to maintain bes_balance
void mongo_db_plugin_impl::update_account(const chain::action& msg) {
   ...
   if (msg.name == transfer) {
      ...
      abi_serializer abis;
      auto besio_account = find_account(accounts, msg.account);
      auto abi = fc::json::from_string(bsoncxx::to_json(besio_account.view()["abi"].get_document())).as<abi_def>();
      abis.set_abi(abi);
      ...
      asset from_balance = asset::from_string(from_account.view()["bes_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["bes_balance"].get_utf8().value.to_string());
      ...
      update_from << "$set" << open_document << "bes_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "bes_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The queue size between nodbes and MongoDB plugin thread.")
