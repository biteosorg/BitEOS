   ...
   try {
      ...
      if( account ) {
         ...
         try {
            ...
            ilog( "Unable to convert BES JSON to MongoDB JSON: ${e}", ("e", e.what()));
            ilog( "  BES JSON: ${j}", ("j", json));
      ...
      if (act.name != "onblock") { // besio::onblock not in original besio.system abi
      ...
      if (actions_to_write) {
         try {
            if( !bulk_actions.execute() ) {
               BES_ASSERT( false, chain::mongo_db_insert_fail, "Bulk actions insert failed for transaction: ${id}", ("id", trx_id_str));
            }
         } catch(...) {
