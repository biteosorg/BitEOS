 ...
 */
#include <besio/sql_db_plugin/sql_db_plugin.hpp>
...
void sql_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
    ...
    cfg.add_options()
            (BUFFER_SIZE_OPTION, bpo::value<uint>()->default_value(256),
             "The queue size between nodbes and SQL DB plugin thread.")
...
void sql_db_plugin::plugin_initialize(const variables_map& options)
{
    ...
    try {
        ...
        if( uri_str.empty()) {
            wlog( "db URI not specified => besio::sql_db_plugin disabled." );
            return;
        }
        ...
        BES_ASSERT( chain_plug, missing_chain_plugin_exception, "" );
