 ...
 */
#include <besio/wallet_plugin/wallet_plugin.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("besio-key", bpo::value<std::string>(),
          "besio key that will be imported automatically when a wallet is created.")
         ;
}
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.count("besio-key")) {
      std::string besio_wif_key = options.at("besio-key").as<std::string>();
      wallet_manager_ptr->set_besio_key(besio_wif_key);
   }
}
} // namespace besio
