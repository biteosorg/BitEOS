...
CHAINBASE_SET_INDEX_TYPE(besio::account_history_object, besio::account_history_index)
CHAINBASE_SET_INDEX_TYPE(besio::action_history_object, besio::action_history_index)
      ...
      try {
         ...
         if( options.count( "filter-out" )) {
            ...
            for( auto& s : fo ) {
               std::vector<std::string> v;
               boost::split( v, s, boost::is_any_of( ":" ));
               BES_ASSERT( v.size() == 3, fc::invalid_arg_exception, "Invalid value ${s} for --filter-out", ("s", s));
               filter_entry fe{v[0], v[1], v[2]};
               BES_ASSERT( fe.receiver.value, fc::invalid_arg_exception,
