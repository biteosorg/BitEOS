...
#include <besio/bnet_plugin/bnet_plugin.hpp>
...
using besio::public_key_type;
using besio::chain_id_type;
using besio::block_id_type;
using besio::block_timestamp_type;
using std::string;
using besio::sha256;
using besio::signed_block_ptr;
using besio::packed_transaction_ptr;
using std::vector;
...
namespace besio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              BES_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              BES_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           BES_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         BES_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
