	...
	if [ "${MEM_GIG}" -lt ${MEM_MIN}/1024 ]; then
		printf "\\tYour system must have %sMgb or more Gigabytes of physical memory installed.\\n" "${MEM_MIN}"
		printf "\\texiting now.\\n"
		exit 1
	fi

	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or higher to install BESIO."
		echo "Exiting now."
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install BESIO."
		echo "Exiting now."
		exit 1
	fi
	...
	then
		printf "\\tHomebrew must be installed to compile BitEOS\\n\\n"
	...
	if [ $COUNT -gt 1 ]; then
		printf "\\n\\tThe following dependencies are required to install BESIO.\\n"
		printf "\\n\\t%s\\n\\n" "${DISPLAY}"
	...
	if [ "${BVERSION}" != "106700" ]; then
		if [ ! -z "${BVERSION}" ]; then
			...
			printf "\\tBitEOS requires Boost version 1.67.\\n"
