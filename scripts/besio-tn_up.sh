#!/bin/bash
#
# besio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# besio-tn_bounce.sh and besio-tn_roll.sh scripts.

connected="0"

rundir=programs/nodbes
prog=nodbes
...
datadir=var/lib/node_$BESIO_NODE
...
relaunch() {
    echo "$rundir/$prog $* --data-dir $datadir --config-dir etc/besio/node_$BESIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $* --data-dir $datadir --config-dir etc/besio/node_$BESIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
