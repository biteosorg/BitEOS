...
namespace besio_system {

class besio_system_tester : public TESTER {
public:

   besio_system_tester()
   : besio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   besio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(besio.token), N(besio.ram), N(besio.ramfee), N(besio.stake),
               N(besio.bpay), N(besio.vpay), N(besio.saving), N(besio.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(besio.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("besio")  + get_balance("besio.ramfee") + get_balance("besio.stake") + get_balance("besio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& besin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",besin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(besio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = besio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(besio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(besio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "besio", "besio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "besio", "alice1111111", core_from_string("650000000.0000"), "besio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::besio_code_name}, 1},
...
inline uint64_t M( const string& bes_str ) {
   return core_from_string( bes_str ).get_amount();
}
