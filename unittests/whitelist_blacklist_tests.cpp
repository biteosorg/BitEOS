...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(besio.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(besio.token), besio_token_wast);
         chain->set_abi(N(besio.token), besio_token_abi);
         chain->push_action( N(besio.token), N(create), N(besio.token), mvo()
              ( "issuer", "besio.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(besio.token), N(issue), N(besio.token), mvo()
              ( "to", "besio.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(besio.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(besio.token), N(alice)};
   test.init();

   test.transfer( N(besio.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(besio.token), N(bob)};
   test.init();

   test.transfer( N(besio.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(besio.token) );
   ...
   test.chain->set_code(N(bob), besio_token_wast);
   test.chain->set_abi(N(bob), besio_token_abi);

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), besio_token_wast);
   test.chain->set_abi(N(charlie), besio_token_abi);
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(besio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_besio ) { try {
   whitelist_blacklist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, besio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'besio::onerror' is on the action blacklist")
                        );
