...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {N(besio), N(besio.token), N(alice)};
   test.init();
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(besio), N(besio.token), N(bob)};
   test.init();
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {N(besio), N(besio.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_besio ) { try {
   ...
   tester1.chain->set_code(N(besio), besio_token_wast);
   tester1.chain->produce_blocks();
   tester1.shutdown();
   tester1.contract_blacklist = {N(besio)};
   tester1.init(false);
...
BOOST_AUTO_TEST_CASE( blacklist_onerror ) { try {
   ...
   tester1.action_blacklist = {{N(besio), N(onerror)}};
   tester1.init(false);
