...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> optional<abi_serializer> {
      return abi_serializer(besio_contract_abi(abi));
   };
}
...
{ try {
   ...
   abi_serializer abis(besio_contract_abi(abi));
   abis.validate();
...
struct abi_gen_helper {
  ...
  bool generate_abi(const char* source, const char* abi, bool opt_sfs=false) {
    ...
    bool res = runToolOnCodeWithArgs(new find_besio_abi_macro_action(contract, actions, ""), source,
...
{ try {
   ...
   abi_serializer abis(besio_contract_abi(abi_def()));
...
{ try {
   ...
   abi_serializer abis(besio_contract_abi(v.as<abi_def>()));
