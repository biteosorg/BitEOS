...
public:
   ...
   fc::variant get_global_state() {
      vector<char> data = get_row_by_account( N(besio), N(besio), N(global), N(global) );
      if (data.empty()) std::cout << "\nData is empty\n" << std::endl;
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "besio_global_state", data, abi_serializer_max_time );

   }

    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(besio), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(besio), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    auto register_producer(name producer) {
       auto r = base_tester::push_action(N(besio), N(regproducer), producer, mvo()
    ...
    auto undelegate_bandwidth( name from, name receiver, asset net, asset cpu ) {
       auto r = base_tester::push_action(N(besio), N(undelegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(besio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create besio.msig and besio.token
        create_accounts({N(besio.msig), N(besio.token), N(besio.ram), N(besio.ramfee), N(besio.stake), N(besio.vpay), N(besio.bpay), N(besio.saving) });

        // Set code for the following accounts:
        //  - besio (code: besio.bios) (already set by tester constructor)
        //  - besio.msig (code: besio.msig)
        //  - besio.token (code: besio.token)
        set_code_abi(N(besio.msig), besio_msig_wast, besio_msig_abi);//, &besio_active_pk);
        set_code_abi(N(besio.token), besio_token_wast, besio_token_abi); //, &besio_active_pk);

        // Set privileged for besio.msig and besio.token
        set_privileged(N(besio.msig));
        set_privileged(N(besio.token));

        // Verify besio.msig and besio.token is privileged
        const auto& besio_msig_acc = get<account_object, by_name>(N(besio.msig));
        BOOST_TEST(besio_msig_acc.privileged == true);
        const auto& besio_token_acc = get<account_object, by_name>(N(besio.token));
        BOOST_TEST(besio_token_acc.privileged == true);


        // Create SYS tokens in besio.token, set its manager as besio
        ...
        create_currency(N(besio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to besio.system
        issue(N(besio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(besio) );
        }

        // Set besio.system to besio
        set_code_abi(N(besio), besio_system_wast, besio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(besio), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(besio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        auto votepro = [&]( account_name voter, vector<account_name> producers ) {
          ...
          base_tester::push_action(N(besio), N(voteproducer), voter, mvo()
                                ("voter",  name(voter))
                                ("proxy", name(0) )
                                ("producers", producers)
                     );
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "besio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), besio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), besio_assert_message_exception);
