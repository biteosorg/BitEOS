...
FC_REFLECT(account_t, (owner)(bes_balance)(open_offers)(open_games));
...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(besio.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(besio.token), N(create), N(besio.token), mvo()
     ("issuer", "besio.token")
     ("maximum_supply", core_from_string("1000000000.0000"))
   );

   push_action(N(besio.token), N(issue), N(besio.token), mvo()
     ("to", "besio")
     ("quantity", core_from_string("1000000000.0000"))
     ("memo", "")
   );

   transfer( config::system_account_name, N(alice), core_from_string("10000.0000"), "", N(besio.token) );
   transfer( config::system_account_name, N(bob),   core_from_string("10000.0000"), "", N(besio.token) );
   transfer( config::system_account_name, N(carol), core_from_string("10000.0000"), "", N(besio.token) );
   ...
   BOOST_REQUIRE_EQUAL(
      get_currency_balance(N(besio.token), symbol(CORE_SYMBOL), N(alice)),
