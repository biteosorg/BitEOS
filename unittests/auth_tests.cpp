...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for besio reqauth action with alice's spending key
   chain.link_authority("alice", "besio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "besio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with besio reqauth
   chain.unlink_authority("alice", "besio", "reqauth");
   ...
   // Link authority for any besio action with alice's scud key
   chain.link_authority("alice", "besio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "besio", "first",  "reqauth");
...
try {
   ...
   // Creating account with besio. prefix with privileged account
   chain.create_account("besio.test1");

   // Creating account with besio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("besio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'besio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "besio", "besio.any", "reqauth" );
   chain.link_authority( "bob", "besio", "besio.any", "reqauth" );

   /// this should succeed because besio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = besio::chain::resource_limits::resource_usage_object;
   using by_owner = besio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link besio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
