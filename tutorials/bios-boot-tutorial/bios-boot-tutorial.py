...
fastUnstakeSystem = './fast.refund/besio.system/besio.system.wasm'
...
def importKeys():
    run(args.clbes + 'wallet import ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clbes + 'wallet import ' + key)
...
def startNode(nodeIndex, account):
    ...
    cmd = (
        ...
        '    --plugin besio::http_plugin'
        '    --plugin besio::chain_api_plugin'
        '    --plugin besio::producer_plugin' +
        otherOpts)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clbes + 'create account besio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clbes + 'system newaccount --transfer besio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clbes + 'transfer besio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clbes + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clbes + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.clbes + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.clbes + 'get table besio besio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clbes + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
...
def proxyVotes(b, e):
    ...
    retry(args.clbes + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clbes + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.clbes + 'push action besio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clbes + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clbes + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'besio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'besio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.clbes + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'besio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clbes + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clbes + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clbes + 'push action besio buyrambytes' + jsonArg(['besio', accounts[0]['name'], 200000]) + '-p besio')
    sleep(1)
...
def stepKillAll():
    run('killall kbesd nodbes || true')
    sleep(1.5)
def stepStartWallet():
    startWallet()
    importKeys()
def stepStartBoot():
    startNode(0, {'name': 'besio', 'pvt': args.private_key, 'pub': args.public_key})
    sleep(1.5)
def stepInstallSystemContracts():
    run(args.clbes + 'set contract besio.token ' + args.contracts_dir + 'besio.token/')
    run(args.clbes + 'set contract besio.msig ' + args.contracts_dir + 'besio.msig/')
def stepCreateTokens():
    run(args.clbes + 'push action besio.token create \'["besio", "10000000000.0000 %s"]\' -p besio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.clbes + 'push action besio.token issue \'["besio", "%s", "memo"]\' -p besio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.clbes + 'set contract besio ' + args.contracts_dir + 'besio.system/')
    sleep(1)
    run(args.clbes + 'push action besio setpriv' + jsonArg(['besio.msig', 1]) + '-p besio@active')
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-besio/stderr')
...
commands = [
    ('k', 'kill',           stepKillAll,                True,    "Kill all nodbes and kbesd processes"),
    ('w', 'wallet',         stepStartWallet,            True,    "Start kbesd, create wallet, fill with keys"),
    ('b', 'boot',           stepStartBoot,              True,    "Start boot node"),
    ('s', 'sys',            createSystemAccounts,       True,    "Create system accounts (besio.*)"),
    ...
    ('q', 'resign',         stepResign,                 True,    "Resign besio"),
...
parser.add_argument('--symbol', metavar='', help="The besio.system symbol", default='SYS')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clbes')
