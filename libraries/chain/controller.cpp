...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
   ...
   void initialize_producer() {
      for (auto producer : conf.genesis.initial_producer_list) {
         ...
         db.db_store_i64(N(besio), N(besio), N(bps), name, pk, data.data(), data.size() );
      }
   }
   ...
   void initialize_chain_emergency(){
      ...
      auto pk = obj.primary_key();
      auto db = memory_db(self);
      db.db_store_i64(N(besio), N(besio), N(chainstatus), N(besio), pk, data.data(), data.size() );
   }
   ...
   void initialize_code(const bytes code) {
     ...
     const auto& account = db.get<account_object,by_name>(N(besio));
     ...
     const auto& account_sequence = db.get<account_sequence_object, by_name>(N(besio));
   ...
   void initialize_token(const bytes code, const bytes abi) {
     auto code_id = fc::sha256::hash( code.data(), (uint32_t)code.size() );
     const auto& account = db.get<account_object,by_name>(N(besio.token));
     int64_t code_size = code.size();
     db.modify( account, [&]( auto& a ) {
       a.last_code_update = conf.genesis.initial_timestamp;
       a.code_version = code_id;
     ...
     const auto& account_sequence = db.get<account_sequence_object, by_name>(N(besio.token));
     db.modify( account_sequence, [&]( auto& aso ) {
       aso.code_sequence += 1;
     });
   ...
   void initialize_database() {
      ...
      create_native_account( N(besio.token), system_auth, system_auth, false );
  ...
  //    ilog((fc::json::to_pretty_string(*pending->_pending_block_state->block)));
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
    ...
    bool check_chainstatus(){
      const auto * cstatus_tid = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(besio), N(besio), N(chainstatus)));
