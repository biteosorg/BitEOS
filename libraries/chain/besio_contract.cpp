...
#include <besio/chain/permission_link_object.hpp>
...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::besio_code_name ) // virtual besio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         BES_THROW( action_validate_exception,
 ...
 */
void apply_besio_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::besio_auth_scope );
   ...
   BES_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   BES_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   BES_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   BES_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );
...
void apply_besio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_besio_updateauth(apply_context& context) {
   ...
   BES_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   BES_ASSERT( update.permission.to_string().find( "besio." ) != 0, action_validate_exception,
               "Permission names that start with 'besio.' are reserved" );
   BES_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   BES_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      BES_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      BES_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      BES_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      BES_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      BES_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_besio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::besio_auth_scope );
   ...
   BES_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   BES_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
void apply_besio_linkauth(apply_context& context) {
//   context.require_write_lock( config::besio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      BES_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      BES_ASSERT(account != nullptr, account_query_exception,
      ...
      BES_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::besio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         BES_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         BES_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_besio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::besio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   BES_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.trx_context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_besio_canceldelay(apply_context& context) {
