...
#include <besio/chain/authority_checker.hpp>
...
namespace besio { namespace chain {
   ...
   { try {
      BES_ASSERT( !level.actor.empty() && !level.permission.empty(), invalid_permission, "Invalid permission" );
      return _db.find<permission_object, by_owner>( boost::make_tuple(level.actor,level.permission) );
   } BES_RETHROW_EXCEPTIONS( chain::permission_query_exception, "Failed to retrieve permission: ${level}", ("level", level) ) }
                                                                             ...
                                                                             )const
   {
      ...
      if( scope == config::system_account_name ) {
          BES_ASSERT( act_name != updateauth::get_name() &&
      ...
      try {
         ...
         if( *linked_permission == config::besio_any_name )
            return optional<permission_name>();
   ...
   {
      BES_ASSERT( auths.size() == 1, irrelevant_auth_exception,
                  "updateauth action should only have one declared authorization" );
      const auto& auth = auths[0];
      BES_ASSERT( auth.actor == update.account, irrelevant_auth_exception,
      ...
      BES_ASSERT( get_permission(auth).satisfies( *min_permission,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      BES_ASSERT( auth.actor == del.account, irrelevant_auth_exception,
                                                           ...
                                                           )const
   {
      ...
      const auto& auth = auths[0];
      BES_ASSERT( auth.actor == link.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      BES_ASSERT( link.type != updateauth::get_name(),  action_validate_exception,
                  "Cannot link besio::updateauth to a minimum permission" );
      BES_ASSERT( link.type != deleteauth::get_name(),  action_validate_exception,
                  "Cannot link besio::deleteauth to a minimum permission" );
      BES_ASSERT( link.type != linkauth::get_name(),    action_validate_exception,
                  "Cannot link besio::linkauth to a minimum permission" );
      BES_ASSERT( link.type != unlinkauth::get_name(),  action_validate_exception,
                  "Cannot link besio::unlinkauth to a minimum permission" );
      BES_ASSERT( link.type != canceldelay::get_name(), action_validate_exception,
                  "Cannot link besio::canceldelay to a minimum permission" );

      const auto linked_permission_name = lookup_minimum_permission(link.account, link.code, link.type);

      if( !linked_permission_name ) // if action is linked to besio.any permission
         return;

      BES_ASSERT( get_permission(auth).satisfies( get_permission({link.account, *linked_permission_name}),
                                                  _db.get_index<permission_index>().indices()              ),
                  irrelevant_auth_exception,
                                                             ...
                                                             )const
   {
      ...
      const auto& auth = auths[0];
      BES_ASSERT( auth.actor == unlink.account, irrelevant_auth_exception,
                  "the owner of the linked permission needs to be the actor of the declared authorization" );

      const auto unlinked_permission_name = lookup_linked_permission(unlink.account, unlink.code, unlink.type);
      BES_ASSERT( unlinked_permission_name.valid(), transaction_exception,
      ...
      if( *unlinked_permission_name == config::besio_any_name )
         return;

      BES_ASSERT( get_permission(auth).satisfies( get_permission({unlink.account, *unlinked_permission_name}),
                                                  _db.get_index<permission_index>().indices()                  ),
                  irrelevant_auth_exception,
                                                                          ...
                                                                          )const
   {
      ...
      BES_ASSERT( get_permission(auth).satisfies( get_permission(cancel.canceling_auth),
                                                  _db.get_index<permission_index>().indices() ),
                  irrelevant_auth_exception,
      ...
      BES_ASSERT( itr != generated_index.end() && itr->sender == account_name() && itr->trx_id == trx_id,
                  tx_not_found,
      ...
      BES_ASSERT( found, action_validate_exception,
                                             ...
                                             )const
   {
      ...
      for( const auto& act : actions ) {
         ...
         for( const auto& declared_auth : act.authorization ) {
            ...
            if( !special_case ) {
               ...
               if( min_permission_name ) { // since special cases were already handled, it should only be false if the permission is besio.any
                  const auto& min_permission = get_permission({declared_auth.actor, *min_permission_name});
                  BES_ASSERT( get_permission(declared_auth).satisfies( min_permission,
                                                                       _db.get_index<permission_index>().indices() ),
                              irrelevant_auth_exception,
      ...
      for( const auto& p : permissions_to_satisfy ) {
         ...
         BES_ASSERT( checker.satisfied( p.first, p.second ), unsatisfied_authorization,
      ...
      if( !allow_unused_keys ) {
         BES_ASSERT( checker.all_keys_used(), tx_irrelevant_sig,
                                             ...
                                             )const
   {
      ...
      BES_ASSERT( checker.satisfied({account, permission}), unsatisfied_authorization,
                                                                     ...
                                                                     )const
   {
      ...
      for (const auto& act : trx.actions ) {
         for (const auto& declared_auth : act.authorization) {
            BES_ASSERT( checker.satisfied(declared_auth), unsatisfied_authorization,
