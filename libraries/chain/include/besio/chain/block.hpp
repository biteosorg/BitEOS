#pragma once
#include <besio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( besio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(besio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(besio::chain::transaction_receipt, (besio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(besio::chain::signed_block, (besio::chain::signed_block_header), (transactions)(block_extensions) )
