...
#define N(X) besio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<besio::chain::name> : private hash<uint64_t> {
      typedef besio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const besio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, besio::chain::name& check);
} // fc


FC_REFLECT( besio::chain::name, (value) )
