...
namespace besio { namespace chain { namespace config {
...
const static uint64_t null_account_name      = N(besio.null);
const static uint64_t producers_account_name = N(besio.prods);
...
const static uint64_t besio_auth_scope       = N(besio.auth);
const static uint64_t besio_all_scope        = N(besio.all);
...
} } } // namespace besio::chain::config

constexpr uint64_t BES_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / besio::chain::config::percent_100;
}

template<typename Number>
Number BES_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + besio::chain::config::percent_100 - besio::chain::config::percent_1)  / besio::chain::config::percent_100;
}
