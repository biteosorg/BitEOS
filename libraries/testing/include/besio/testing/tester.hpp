...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::besio::testing::filter_fields( a.get_object(), b.get_object() ); \
...
namespace besio { namespace testing {
  ...
  /**
   * Utility predicate to check whether an besio_assert message is equivalent to a given string
   */
  struct besio_assert_message_is {
     besio_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const besio_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an besio_assert message starts with a given string
   */
  struct besio_assert_message_starts_with {
     besio_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an besio_assert_code error code is equivalent to a given number
   */
  struct besio_assert_code_is {
     besio_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const besio_assert_code_exception& ex );
