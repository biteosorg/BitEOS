...
void
besd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string besio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), besio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (besd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (besd_def &node, host_def *host);
  void launch (besd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, besd_def> find_node(uint16_t node_num);
  vector<pair<host_def, besd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodbes does not require transaction signatures.")
    ("nodbes", bpo::value<string>(&besd_extra_args), "forward nodbes command line argument(s) to each instance of nodbes, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodbes instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &besd : binding.instances) {
          besd.host = binding.host_name;
          besd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(besd.p2p_port);

          aliases.push_back (besd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("BESIO_HOME");
...
void
launcher_def::assign_name (besd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.besio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      besd_def besd;
      assign_name(besd, i == 0);
      aliases.push_back(besd.name);
      besd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(besd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->besio_home =
          (local_id.contains (lhost->host_name) || servers.default_besio_home.empty()) ?
          erd : servers.default_besio_home;
        host_ndx++;
      } // ph_count == 0

      besd_def besd;
      assign_name(besd, do_bios);
      besd.has_db = false;
      ...
      aliases.push_back(besd.name);
      besd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(besd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  besd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->besio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->besio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->besio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->besio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->besio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = besio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = besio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = besio::net_plugin\n";
  } else {
    cfg << "plugin = besio::bnet_plugin\n";
  }
  cfg << "plugin = besio::chain_api_plugin\n"
      << "plugin = besio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      besio::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (besd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->besio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->besio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->besio_home;

  cmd = "cd " + host->besio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->besio_home
...
void
launcher_def::launch (besd_def &instance, string &gts) {
  ...
  string besdcmd = "programs/nodbes/nodbes ";
  if (skip_transaction_signatures) {
    besdcmd += "--skip-transaction-signatures ";
  }
  if (!besd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +besio::mongo_db_plugin");
       string args = std::regex_replace (besd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       besdcmd += args + " ";
    }
    else {
       besdcmd += besd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    besdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  besdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  besdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    besdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->besio_home + "; nohup " + besdcmd + " > "
    ...
    string cmd = "cd " + host->besio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << besdcmd << endl;

    bp::child c(besdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << besdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(besd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, besd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, besd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const besd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.besio_home + "; "
                 + "export BESIO_HOME=" + host.besio_home + string("; ")
                 + "export BESIO_NODE=" + node_num + "; "
                 + "./scripts/besio-tn_bounce.sh " + besd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.besio_home + "; "
                 + "export BESIO_HOME=" + host.besio_home + "; "
                 + "export BESIO_NODE=" + node_num + "; "
         + "export BESIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/besio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the besio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the besio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the besio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << besio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(besio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (besio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_besio_home) (ssh) )
