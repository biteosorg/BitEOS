 ...
 *  @defgroup besclienttool BESIO Command Line Client Reference
 *  @brief Tool for sending transactions and querying state from @ref nodbes
 *  @ingroup besclienttool
 */

/**
  @defgroup besclienttool

  @section intro Introduction to clbes

  `clbes` is a command line tool that interfaces with the REST api exposed by @ref nodbes. In order to use `clbes` you will need to
  have a local copy of `nodbes` running and configured to load the 'besio::chain_api_plugin'.

   clbes contains documentation for all of its commands. For a list of all commands known to clbes, simply run it with no arguments:
```
$ ./clbes
Command Line Interface to BESIO Client
Usage: programs/clbes/clbes [OPTIONS] SUBCOMMAND

Options:
  -h,--help                   Print this help message and exit
  -u,--url TEXT=http://localhost:8888/
                              the http/https URL where nodbes is running
  --wallet-url TEXT=http://localhost:8888/
                              the http/https URL where kbesd is running
...
Subcommands:
  ...
  transfer                    Transfer BES from account to account
...
Usage: ./clbes create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain

$ ./clbes create account
Create a new account on the blockchain
Usage: ./clbes create account [OPTIONS] creator name OwnerKey ActiveKey
...
#include <besio/chain/config.hpp>
#include <besio/chain/wast_to_wasm.hpp>
...
using namespace besio::utilities;
using namespace besio::client::help;
using namespace besio::client::http;
using namespace besio::client::localize;
using namespace besio::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION( explained_exception, 9000000, "explained exception, see error log" );
FC_DECLARE_EXCEPTION( localized_exception, 10000000, "an error occured" );
#define BESC_ASSERT( TEST, ... ) \
  FC_EXPAND_MACRO( \
...
besio::client::http::http_context context;
...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      auto sp = std::make_unique<besio::client::http::connection_param>(context, parse_url(url) + path, no_verify ? false : true, headers);
      return besio::client::http::do_http_call(*sp, fc::variant(v), print_request, print_response );
   }
   catch(boost::system::system_error& e) {
      if(url == ::url)
         std::cerr << localized("Failed to connect to nodbes at ${u}; is nodbes running?", ("u", url)) << std::endl;
      else if(url == ::wallet_url)
         std::cerr << localized("Failed to connect to kbesd at ${u}; is kbesd running?", ("u", url)) << std::endl;
...
besio::chain_apis::read_only::get_info_results get_info() {
   return call(url, get_info_func).as<besio::chain_apis::read_only::get_info_results>();
}
...
fc::variant push_transaction( signed_transaction& trx, int32_t extra_kcpu = 1000, packed_transaction::compression_type compression = packed_transaction::none ) {
   ...
   if (trx.signatures.size() == 0) { // #5445 can't change txn content if already signed
      ...
      } BES_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
auto abi_serializer_resolver = [](const name& account) -> optional<abi_serializer> {
   ...
   if ( it == abi_cache.end() ) {
      ...
      auto abi_results = result.as<besio::chain_apis::read_only::get_abi_results>();
...
bytes json_or_file_to_bin( const account_name& account, const action_name& action, const string& data_or_filename ) {
   ...
   if( !data_or_filename.empty() ) {
      ...
      } BES_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data_or_filename));
   }
...
chain::action create_newaccount(const name& creator, const name& newaccount, public_key_type owner, public_key_type active) {
   return action {
      ...
      besio::chain::newaccount{
         .creator      = creator,
         .name         = newaccount,
         .owner        = besio::chain::authority{1, {{owner, 1}}, {}},
         .active       = besio::chain::authority{1, {{active, 1}}, {}}
      }
   };
}
...
fc::variant regproducer_variant(const account_name& producer, const public_key_type& key, const string& url, uint16_t location, uint32_t commission_rate) {
...
authority parse_json_authority(const std::string& authorityJsonOrFile) {
   ...
   } BES_RETHROW_EXCEPTIONS(authority_type_exception, "Fail to parse Authority JSON '${data}'", ("data",authorityJsonOrFile))
}

authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "BES") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
         return authority(public_key_type(authorityJsonOrFile));
      } BES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", authorityJsonOrFile))
   } else {
      auto result = parse_json_authority(authorityJsonOrFile);
      BES_ASSERT( besio::chain::validate(result), authority_type_exception, "Authority failed validation! ensure that keys, accounts, and waits are sorted and that the threshold is valid and satisfiable!");
      return result;
   }
}

asset to_asset( const string& code, const string& s ) {
   static map<besio::chain::symbol_code, besio::chain::symbol> cache;
   auto a = asset::from_string( s );
   besio::chain::symbol_code sym = a.get_symbol().to_symbol_code();
   ...
   if ( it == cache.end() ) {
      ...
      if (obj_it != obj.end()) {
         auto result = obj_it->value().as<besio::chain_apis::read_only::get_currency_stats_result>();
         auto p = cache.insert(make_pair( sym, result.max_supply.get_symbol() ));
         it = p.first;
      } else {
         BES_THROW(symbol_type_exception, "Symbol ${s} is not supported by token contract ${c}", ("s", sym_str)("c", code));
      }
   }
   ...
   if ( a.decimals() < expected_symbol.decimals() ) {
      ...
      BES_THROW(symbol_type_exception, "Too many decimal digits in ${a}, only ${d} supported", ("a", a)("d", expected_symbol.decimals()));
   } // else precision matches
   return a;
}

inline asset to_asset( const string& s ) {
   return to_asset( "besio.token", s );
}
...
void try_local_port( const string& lo_address, uint16_t port, uint32_t duration ) {
   ...
   while ( !local_port_used(lo_address, port)) {
      if (duration_cast<std::chrono::milliseconds>( system_clock::now().time_since_epoch()).count() - start_time > duration ) {
         std::cerr << "Unable to connect to kbesd, if kbesd is running please kill the process and try again.\n";
         throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to kbesd")});
      }
   }
}

void ensure_kbesd_running(CLI::App* app) {
    // get, version, net do not require kbesd
    ...
    for (const auto& addr: resolved_url.resolved_addresses)
       if (local_port_used(addr, resolved_url.resolved_port))  // Hopefully taken by kbesd
          return;

    boost::filesystem::path binPath = boost::dll::program_location();
    binPath.remove_filename();
    // This extra check is necessary when running clbes like this: ./clbes ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if clbes and kbesd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kbesd").append(key_store_executable_name);
    }
    ...
    if (boost::filesystem::exists(binPath)) {
        ...
        ::boost::process::child kbes(binPath, pargs,
                                     bp::std_in.close(),
                                     bp::std_out > bp::null,
                                     bp::std_err > bp::null);
        if (kbes.running()) {
        ...
        std::cerr << "No wallet service listening on " << lo_address << ":" << std::to_string(resolved_url.resolved_port)
                  << ". Cannot automatically start kbesd because kbesd was not found." << std::endl;
    }
}
...
struct register_producer_subcommand {
   ...
   string url;
   uint16_t loc = 0;
   uint32_t commission_rate = 1000; //%10
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->add_option("commission_rate", commission_rate, localized("Node supports voter commission rate"), true);
      ...
      register_producer->set_callback([this] {
         ...
         } BES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake-net", stake_net,
                                   (localized("The amount of BES delegated for net bandwidth")))->required();
         createAccount->add_option("--stake-cpu", stake_cpu,
                                   (localized("The amount of BES delegated for CPU bandwidth")))->required();
         ...
         createAccount->add_option("--buy-ram", buy_ram_bes,
                                   (localized("The amount of RAM bytes to purchase for the new account in BES")));
         createAccount->add_flag("--transfer", transfer,
                                 (localized("Transfer voting power and right to unstake BES to receiver")));
      }
      ...
      createAccount->set_callback([this] {
            ...
            } BES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid owner public key: ${public_key}", ("public_key", owner_key_str));
            try {
               active_key = public_key_type(active_key_str);
            } BES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid active public key: ${public_key}", ("public_key", active_key_str));
            auto create = create_newaccount(creator, account_name, owner_key, active_key);
            if (!simple) {
               if ( buy_ram_bes.empty() && buy_ram_bytes_in_kbytes == 0) {
                  std::cerr << "ERROR: Either --buy-ram or --buy-ram-kbytes with non-zero value is required" << std::endl;
                  return;
               }
               action buyram = !buy_ram_bes.empty() ? create_buyram(creator, account_name, to_asset(buy_ram_bes))
...
struct vote_producers_subcommand {
   ...
   vote_producers_subcommand(CLI::App* actionRoot) {
      auto vote_producers = actionRoot->add_subcommand("prods", localized("Vote for one producer"));
      vote_producers->add_option("voter", voter_str, localized("The voting account"))->required();
      vote_producers->add_option("producer", producer_str, localized("The account to vote for."))->required();
      vote_producers->add_option("votes", votes_str, localized("The number of votes cast for producers."))->required();
...
struct approve_producer_subcommand {
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            ...
            auto res = result.as<besio::chain_apis::read_only::get_table_rows_result>();
            ...
            BES_ASSERT( 1 == res.rows.size(), multiple_voter_info, "More than one voter_info for account" );
            auto prod_vars = res.rows[0]["producers"].get_array();
            vector<besio::name> prods;
...
struct list_producers_subcommand {
   ...
   list_producers_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         ...
         auto result = rawResult.as<besio::chain_apis::read_only::get_producers_result>();
         ...
         for ( auto& row : result.rows )
            printf("%-13.13s %-54.54s %-59.59s %1.4f\n",
                   ...
                   row["total_votes"].as_double() / result.total_activated_votes);
...
struct get_transaction_id_subcommand {
   ...
   get_transaction_id_subcommand(CLI::App* actionRoot) {
      ...
      get_transaction_id->set_callback([&] {
         ...
         } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_check))
      });
   }
};
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of BES to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of BES to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of BES to buyram"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake BES to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of BES to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of BES to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of BES to bid"))->required();
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "besio")("scope", "besio")("table", "namebids")
                               ("lower_bound", besio::chain::string_to_name(newname_str.c_str()))("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
         }
         auto result = rawResult.as<besio::chain_apis::read_only::get_table_rows_result>();
         if ( result.rows.empty() ) {
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of BES to pay for RAM, or number of kbytes of RAM if --kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive BES for sold RAM"))->required();
...
struct claimrewards_subcommand {
   ...
   claimrewards_subcommand(CLI::App* actionRoot) {
      auto claim_rewards = actionRoot->add_subcommand("claimrewards", localized("Claim producer or voters rewards"));
      claim_rewards->add_option("owner", owner, localized("The account to claim rewards for"))->required();
      claim_rewards->add_option("producer", producer, localized("The account to claim rewards form producer, if claim producer, owner and producer is same"))->required();
...
void get_account( const string& accountName, bool json_format ) {
   ...
   auto res = json.as<besio::chain_apis::read_only::get_account_results>();
   ...
   if (!json_format) {
      ...
      unordered_map<name, besio::chain_apis::permission> cache;
      ...
      if ( res.total_resources.is_object() ) {
         std::cout << indent << "trading fee rate:  " << std::setw(46) << double( res.total_resources.get_object()["ram_trading_fee_rate"].as_uint64() ) / 100 << " \%" << std::endl;
         
         auto time_string = res.total_resources.get_object()["ram_last_warning_time"];
         if ( 0 == time_string.as_uint64()) {
            std::cout << indent << "last warning time:  " << std::setw(47) << "not available" << std::endl;
         } else {
            fc::time_point time( fc::microseconds( time_string.as_uint64() ) );
            std::cout << indent << "last warning time:  " << std::setw(47) << ((std::string)time).c_str() << std::endl;
         }

         time_string = res.total_resources.get_object()["ram_info_last_update_time"];
         if ( 0 == time_string.as_uint64()) {
            std::cout << indent << "last trading fee rate update time:  " << std::setw(32) << "not available" << std::endl;
         } else {
            fc::time_point time( fc::microseconds( time_string.as_uint64() ) );
            std::cout << indent << "last trading fee rate update time:  " << std::setw(31) << ((std::string)time).c_str() << std::endl;
         }
      }
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of nodbes responding to the request is different than core symbol built into clbes
      ...
      if( res.refund_request.is_object() ) {
         ...
         if( unstaking > asset( 0, unstaking.get_symbol() ) ) {
            ...
            if( now >= refund_time ) {
               std::cout << " (available to claim now with 'besio::refund' action)\n";
            } else {
      ...
      if ( res.voter_info.size() ) {
         ...
         for(auto &voter : res.voter_info){
             ...
             fc::time_point time(fc::microseconds(obj["voteage_update_time"].as_uint64()));

             std::cout << indent << std::left << std::setw(11)    
                       << "voteage update time:" << std::right << std::setw(30) << ((std::string)time).c_str() << std::endl << std::endl;
...
int main( int argc, char** argv ) {
   ...
   context = besio::client::http::create_http_context();

   CLI::App app{"Command Line Interface to BESIO Client"};
   app.require_subcommand();
   app.add_option( "-H,--host", obsoleted_option_host_port, localized("the host where nodbes is running") )->group("hidden");
   app.add_option( "-p,--port", obsoleted_option_host_port, localized("the port where nodbes is running") )->group("hidden");
   app.add_option( "--wallet-host", obsoleted_option_host_port, localized("the host where kbesd is running") )->group("hidden");
   app.add_option( "--wallet-port", obsoleted_option_host_port, localized("the port where kbesd is running") )->group("hidden");

   app.add_option( "-u,--url", url, localized("the http/https URL where nodbes is running"), true );
   app.add_option( "--wallet-url", wallet_url, localized("the http/https URL where kbesd is running"), true );

   app.add_option( "-r,--header", header_opt_callback, localized("pass specific HTTP header; repeat this option to pass multiple headers"));
   app.add_flag( "-n,--no-verify", no_verify, localized("don't verify peer certificate when using HTTPS"));
   app.set_callback([&app]{ ensure_kbesd_running(&app);});
   ...
   version->add_subcommand("client", localized("Retrieve version information of the client"))->set_callback([] {
     std::cout << localized("Build version: ${ver}", ("ver", besio::client::config::version_str)) << std::endl;
   });
   ...
   pack_transaction->add_flag("--pack-action-data", pack_action_data_flag, localized("Pack all action data within transaction, needs interaction with nodbes"));
   pack_transaction->set_callback([&] {
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string( plain_signed_transaction_json );
      } BES_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse plain transaction JSON '${data}'", ("data", plain_signed_transaction_json))
      if( pack_action_data_flag ) {
         ...
         } BES_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to convert transaction, --pack-action-data likely needed" )
      }
   });
   ...
   unpack_transaction->add_flag("--unpack-action-data", unpack_action_data_flag, localized("Unpack all action data within transaction, needs interaction with nodbes"));
   unpack_transaction->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS( transaction_type_exception, "Fail to parse packed transaction JSON '${data}'", ("data", packed_transaction_json))
   ...
   pack_action_data->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse unpacked action data JSON")
   ...
   unpack_action_data->set_callback([&] {
      BES_ASSERT( packed_action_data_string.size() >= 2, transaction_type_exception, "No packed_action_data found" );
   ...
   getCode->set_callback([&] {
      ...
      try {
         ...
         //see if this is an old nodbes that doesn't support get_raw_code_and_abi
         ...
         if(code_as_wasm) {
            ...
            std::cout << localized("Warning: communicating to older nodbes which returns malformed binary wasm") << std::endl;
         }
         else
   ...
   getAccounts->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", public_key_str))
   ...
   getTransaction->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_id_type_exception, "Invalid transaction ID: ${transaction_id}", ("transaction_id", transaction_id_str))
   ...
   getTransactions->set_callback([&] {
      ...
      if (skip_seq_str.empty()) {
         ...
         } BES_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Skip Seq: ${skip_seq}", ("skip_seq", skip_seq_str))
         if (num_seq_str.empty()) {
            ...
            } BES_RETHROW_EXCEPTIONS(chain_type_exception, "Invalid Num Seq: ${num_seq}", ("num_seq", num_seq_str))
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      BES_ASSERT( !wasm.empty(), wast_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto set_abi_callback = [&]() {
      ...
      BES_ASSERT( fc::exists( abiPath ), abi_file_not_found, "no abi file found ${f}", ("f", abiPath)  );

      try {
         actions.emplace_back( create_setabi(account, fc::json::from_file(abiPath).as<abi_def>()) );
      } BES_RETHROW_EXCEPTIONS(abi_type_exception,  "Fail to parse ABI JSON")
      if ( shouldSend ) {
   ...
   // Transfer subcommand
   string con = "besio.token";
   string sender;
   string recipient;
   string amount;
   string memo;
   auto transfer = app.add_subcommand("transfer", localized("Transfer BES from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending BES"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving BES"))->required();
   transfer->add_option("amount", amount, localized("The amount of BES to send"))->required();
   ...
   importWallet->set_callback([&wallet_name, &wallet_key_str] {
      ...
      try {
         ...
         BES_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
      }
   ...
   removeKeyWallet->set_callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
      ...
      try {
         ...
         BES_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
      }
   ...
   auto stopKbesd = wallet->add_subcommand("stop", localized("Stop kbesd (doesn't work with nodbes)."), false);
   stopKbesd->set_callback([] {
      const auto& v = call(wallet_url, kbesd_stop);
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success kbesd responds with empty object
   ...
   sign->set_callback([&] {
      ...
      if( str_chain_id.size() == 0 ) {
         ilog( "grabbing chain_id from nodbes" );
         auto info = get_info();
   ...
   actionsSubcommand->set_callback([&] {
      ...
      if( !data.empty() ) {
         ...
         } BES_RETHROW_EXCEPTIONS(action_type_exception, "Fail to parse action JSON data='${data}'", ("data", data))
      }
   ...
   trxSubcommand->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_push))
      try {
         signed_transaction trx = trx_var.as<signed_transaction>();
   ...
   trxsSubcommand->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trxsJson))
   ...
   propose_action->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",requested_perm))
      fc::variant transaction_perm_var;
      try {
         transaction_perm_var = json_from_file_or_string(transaction_perm);
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm))
      fc::variant trx_var;
      try {
         trx_var = json_from_file_or_string(proposed_transaction);
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong requested permissions format: '${data}'", ("data",requested_perm_var));

      vector<permission_level> trxperm;
      try {
         trxperm = transaction_perm_var.as<vector<permission_level>>();
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var));
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            ...
            BES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
      if (proposer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", "propose", variant_to_bin( N(besio.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                         ...
                         ("table", "proposal")
                         ("table_key", "")
                         ("lower_bound", besio::chain::string_to_name(proposal_name.c_str()))
                         ("upper_bound", "")
                         ("limit", 1)
                         );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",perm))
      auto args = fc::mutable_variant_object()
         ("proposer", proposer)
         ("proposal_name", proposal_name)
         ("level", perm_var);
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", action, variant_to_bin( N(besio.msig), action, args ) }});
   };
   ...
   cancel->set_callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            ...
            BES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <canceler> or -p)");
         }
      }
      if (canceler.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", "cancel", variant_to_bin( N(besio.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            ...
            BES_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <executer> or -p)");
         }
      }
      if (executer.empty()) {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", "exec", variant_to_bin( N(besio.msig), N(exec), args ) }});
      }
   );
   ...
   sudo_exec->set_callback([&] {
      ...
      } BES_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",trx_to_exec))

      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{executer, config::active_name}, {"besio.sudo", config::active_name}};
      }
      ...
      send_actions({chain::action{accountPermissions, "besio.sudo", "exec", variant_to_bin( N(besio.sudo), N(exec), args ) }});
   });

   // system subcommand
   auto system = app.add_subcommand("system", localized("Send besio.system contract action to the blockchain."), false);
