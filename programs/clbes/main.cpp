...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      besio::client::http::connection_param *cp = new besio::client::http::connection_param(context, parse_url(url) + path,
              no_verify ? false : true, headers);

      return besio::client::http::do_http_call( *cp, fc::variant(v), print_request, print_response );
   }
...
void ensure_kbesd_running(CLI::App* app) {
    ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append("kbesd"); // if clbes and kbesd are in the same installation directory
    if (!boost::filesystem::exists(binPath)) {
        binPath.remove_filename().remove_filename().append("kbesd").append("kbesd");
    }
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      BES_ASSERT( !wast.empty(), wast_file_not_found, "no wast file found ${f}", ("f", wastPath) );
      vector<uint8_t> wasm;
