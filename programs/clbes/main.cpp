...
fc::variant call( const std::string& url,
                  ...
                  const T& v ) {
   try {
      besio::client::http::connection_param *cp = new besio::client::http::connection_param((std::string&)url, (std::string&)path,
              no_verify ? false : true, headers);

      return besio::client::http::do_http_call( *cp, fc::variant(v), print_request );
   }
...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "BES")) {
      try {
...
void ensure_kbesd_running(CLI::App* app) {
    ...
    if (port_used(uint16_t(wallet_port)))  // Hopefully taken by kbesd
        return;
...
struct vote_producers_subcommand {
   string voter_str;
   vector<besio::name> producer_names;
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("tokens", amount, localized("The amount of BES to pay for RAM"))->required();
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", "propose", result.get_object()["binargs"].as<bytes>()}});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", action, result.get_object()["binargs"].as<bytes>()}});
   };
   ...
   cancel->set_callback([&]() {
      ...
      auto result = call(json_to_bin_func, arg);
      send_actions({chain::action{accountPermissions, "besio.msig", "cancel", result.get_object()["binargs"].as<bytes>()}});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "besio.msig", "exec", result.get_object()["binargs"].as<bytes>()}});
      }
   );
